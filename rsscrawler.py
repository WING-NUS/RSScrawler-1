...
def replaceAll4FileName(oldName):
...
def loadWordsFrequency ():
	global ppath, RSSName, wordsFreq

	if not os.path.exists(ppath + RSSName+'_wordsfreq'+'.db'):
		conn = sqlite3.connect(ppath + RSSName+'_wordsfreq'+'.db')
		c = conn.cursor()
		...
		for line in c.execute('SELECT * FROM wordsfreq'):
			if line[0] not in wordsFreq.keys():
				wordsFreq[line[0]]={line[2]:int(line[3])}
			elif line[2] not in wordsFreq[line[0]].keys():
				wordsFreq[line[0]][line[2]] = int(line[3])
			else:
				wordsFreq[line[0]][line[2]] += int(line[3])
		c.close()			
...
def calWordsFrequency(content, daytime):
	global wordsFreq, stopWords
	...
	if ( daytime not in set(wordsFreq.keys())):
		wordsFreq[daytime] = {}
	for word in words:
		if (word not in set(stopWords)):
			if (len(wordsFreq[daytime].keys()) == 0) or (word not in set(wordsFreq[daytime].keys())):
				wordsFreq[daytime][word] = 1
			else:
				wordsFreq[daytime][word] += 1

# load all fetched links from .db file
def loadAllFetchedLinks(fileName):
	global ppath

	links = []
	if not os.path.exists(ppath + fileName +'.db'):
		conn = sqlite3.connect(ppath + fileName +'.db')
		c = conn.cursor()
...
def updateNewLinks(fileName, newlinks):
	global ppath
...
def getNameAndLanguageFromResource(rssResource):
	global RSSName
...
def storeNewLinkInMERGEandHTML(rssResource, page, title, link, date):
	global  ppath

	dd = replaceAll4FileName(title)
	file_id = int(time.time() * 1000)
	# print "ppath = "+ppath
	try:
		myFile = open(ppath  + str(file_id)+'.'+dd+'.html', 'w')
		myFile.write(page)
		(sourcename,language) = getNameAndLanguageFromResource(rssResource)
		...
		myFile2 = open(ppath  + 'MERGE.TXT', 'a')
...
def updateWordsFrequency():
	global ppath, RSSName, wordsFreq

	conn = sqlite3.connect(ppath + RSSName+'_wordsfreq'+'.db')
	c = conn.cursor()
	c.execute('DELETE FROM wordsfreq')
	for key, value in sorted(wordsFreq.iteritems(), key=lambda t: t[0]):
		for key2, value2 in sorted(value.iteritems(), key=lambda t: t[1], reverse=True):
				...
				try:
					c.execute('INSERT INTO wordsfreq VALUES (?,?,?,?)', (key, RSSName, key2, value2))
	...
	myFile = codecs.open( ppath + RSSName+'.txt',encoding='utf-8', mode='w')
	...
	for key, value in sorted(wordsFreq.iteritems(), key=lambda t: t[0]):
		for key2, value2 in sorted(value.iteritems(), key=lambda t: t[1], reverse=True):
				...
				if type(key2) is str:
					key2=key2.decode('utf-8')
				try:
					line = key + '	' + RSSName + '	' + key2 + '	'+str(value2)
					myFile.write(line)
				 	myFile.write('\n')
				except UnicodeDecodeError:
					print RSSName+ ".txt file is written error ..."
...
def processArgs(argv):
	global path, ppath, sources, stopWords, RSSName
	...
	if RSSName == '':
		...
		sys.exit(3)
	print 'Input News name is : ', RSSName
...
def fetchNews():
	...
	for rssResource in sources:
		...
		links = loadAllFetchedLinks(fileName)
		...
		while 1:
			...
			i = i+1
			try:
				if (RSSName == 'newyorktimes' ) : # solve redirect problem in terms of NewYork Times RSS webpage fetching
					while 1:
						try:
				...
				if d.entries[i].link not in links:
					...
					value = storeNewLinkInMERGEandHTML(rssResource, page, d.entries[i].title, d.entries[i].link, str(d.entries[i].published_parsed))
					# if (value == "error"):
					# 	continue
					# calculate words frequency
					calWordsFrequency(d.entries[i].title, str(d.entries[i].published_parsed[2]))
		...
		if (len(new_links) !=0) :
			...
			updateNewLinks(fileName, new_links)

			# update words frequency record file
			updateWordsFrequency()
...
if __name__ == '__main__':
	...
	__flags__ = ['newyorktimes', 'economist']
